...
from db.models import DiffCell, DiffCellBatch
...
def main(path):
    """
    Read in the differentiated cell batch info
    """
    sheet_name = 'Sheet1'
    start_row = 1
    ...
    column_definitions = { 
        ...
        'DC_Center_Specific_Code': 'center_specific_code',
        'DC_Provider_Name':'provider_name',
        'DC_Provider_Catalog_ID':'provider_catalog_id',
        'DC_Provider_Batch_ID':'provider_batch_id',
        
        'DC_Source_Information': 'source_information',
        'DC_Date_Received': 'date_received',
        'DC_Quality_Verification':'quality_verification',
        'DC_Culture_Conditions': 'culture_conditions',
        'DC_Passage_Number': (
            'passage_number',False,None,lambda x:util.convertdata(x,int)),
        'DC_Days_Post_Differentiation': (
            'days_post_differentiation',False,None,lambda 
    ...
    for row in sheet:
        ...
        for i,value in enumerate(r):
            ...
            if model_field == 'facility_id':
                try:
                    cell = DiffCell.objects.get(facility_id=value)
                    initializer['reagent'] = cell
                except:
                    logger.exception(
                        "Diff Cell not found: %r, row: %d", value,rows+start_row+1)
                    raise
            else:
                initializer[model_field] = value
        try:
            logger.debug('initializer: %r', initializer)
            cell = DiffCellBatch(**initializer)
            cell.save()
            logger.debug('differentiated cell batch created: %r', cell)
            rows += 1
        except Exception, e:
            logger.exception(
                "Invalid Diff CellBatch initializer: %r, row: %d",
