...
from db.models import EsCell, EsCellBatch
...
def main(path):
    """
    Read in the Embryonic Stem cell batch info
    """
    sheet_name = 'Sheet1'
    start_row = 1
    ...
    column_definitions = { 
        ...
        'ES_Center_Specific_Code': 'center_specific_code',
        'ES_Provider_Name':'provider_name',
        'ES_Provider_Catalog_ID':'provider_catalog_id',
        'ES_Provider_Batch_ID':'provider_batch_id',
        
        'ES_Source_Information': 'source_information',
        'ES_Date_Received': 'date_received',
        'ES_Quality_Verification':'quality_verification',
        'ES_Culture_Conditions': 'culture_conditions',
        'ES_Passage_Number': (
            'passage_number',False,None,lambda x:util.convertdata(x,int)),
        'ES_Transient_Modification': 'transient_modification',
    ...
    for row in sheet:
        ...
        for i,value in enumerate(r):
            ...
            if model_field == 'facility_id':
                try:
                    cell = EsCell.objects.get(facility_id=value)
                    initializer['reagent'] = cell
                except:
                    logger.exception(
                        "EsCell Cell not found: %r, row: %d", value,rows+start_row+1)
                    raise
            else:
                initializer[model_field] = value
        try:
            logger.debug('initializer: %r', initializer)
            cell = EsCellBatch(**initializer)
            cell.save()
            logger.debug('Embryonic Stem cell batch created: %r', cell)
            rows += 1
        except Exception, e:
            logger.exception(
                "Invalid EsCell CellBatch initializer: %r, row: %d",
