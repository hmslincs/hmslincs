...
def main(path):
    ...
    for row in sheet:
        ...
        try:
            ...
            try:
                up = Unclassified.objects.get(facility_id=facility_id)
                initializer['reagent'] = up
            except ObjectDoesNotExist, e:
                logger.error('facility_id: "%s" does not exist, row: %d' 
                    % (facility_id,i))
            batch = UnclassifiedBatch(**initializer)
            batch.save()
            logger.debug('batch created: %s', batch)
            rows += 1
        except Exception, e:
            logger.error("Invalid unclassified perturbagen batch initializer: %s" % initializer)
            raise
