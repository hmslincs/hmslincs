...
from db.models import EsCell, EsCellBatch, PrimaryCellBatch
...
def main(path):
    ...
    column_definitions = {
        ...
        'ES_Name':('name',True),
        'ES_LINCS_ID':'lincs_id',
        'ES_Alternative_Name':'alternative_names',
        'ES_Alternative_ID': 'alternative_id',

        'Precursor_Primary_Cell':'precursor_facility_batch_id',
        
        'ES_Organism': 'organism',
        'ES_Production_Details': 'production_details',
        'ES_Known_Mutations': 'mutations_known',
        'ES_Mutation_Citations': 'mutation_citations',
        'ES_Molecular_Features': 'molecular_features',
        'ES_Recommended_Culture_Conditions': 'recommended_culture_conditions',
        'ES_Disease': 'disease',
        'ES_Disease_Detail': 'disease_detail',
        'ES_Related_Projects': 'related_projects',
        'ES_Cell_Markers': 'cell_markers',
        'ES_Genetic_Modification': 'genetic_modification',
        'ES_Passage_Last_Karyotyping': (
    ...
    for row in sheet:
        ...
        try:
            ...
            esc = EsCell(**initializer)
            esc.save()
            logger.info('embryonic stem cell created: %r', esc)

            # create a default batch - 0
            EsCellBatch.objects.create(reagent=esc,batch_id=0)
            
        except Exception, e:
            print "Invalid Embryonic Cell, name: ", r[0]
            raise e
        
        rows += 1
    print "EsCell Cells read: ", rows
    ...
    for facility_id,(precursor_facility_id,batch_id) in precursor_map.items():
        try:
            ...
            precursor = PrimaryCellBatch.objects.get(
                ...
                batch_id=batch_id)
            esc = EsCell.objects.get(facility_id=facility_id)
            esc.precursor = precursor
            esc.save()
    ...
    print 'EsCell (Primary Cell) Precursor loaded:', len(precursor_map)
